...
export const createLeave = async (req, res) => {
    try {
        ...
        if (!type || !startDate || !endDate || !reason) return res.status(400).json({error: "Missing fields."})
        const today = new Date()
        today.setHours(0, 0, 0, 0)
        if (new Date(startDate) <= today || new Date(endDate) <= today) return res.status(400).json({error: "Leave dates must be in the future"})
        ...
        console.error(error)
        return res.status(500).json({error: "Failed to create a leave"})
    }
}
...
export const getLeaves = async (req, res) => {
    try {
        ...
        if (isAdmin) {
            ...
            const employee = await Employee.find({userId: session.userId,}).lean()
            if (!employee) return res.status(404).json({error: "Employee not found"})
...
export const updateLeaveStatus = async (req, res) => {
    try {
        ...
        const leave = await LeaveApplication.findByIdAndUpdate(req.params.id, {status}, {status}, {returnDocument: "after"})
        return res.json({success: true, data: leave})
    } catch (error) {
        console.error(error)
        return res.status(500).json({error: "Failed to update leaves: "})
    }
}