...
export const inngest = new Inngest({ id: "hui-fullstack-ems" });

// Auto Check-out for employees
const autoCheckOut = inngest.createFunction(
  {id: "auto-check-out", triggers: [{event: "employee/check-out"}]},
  async ({ event, step }) => {
    const {empoyeeId, attendanceId} = event.data

    // Wait for 9 hours
    await step.sleepUtil("wait-for-the-9-hours", new Date().getTime() + 9 * 60 * 60 * 1000);
    ...
    if (!attendance?.checkOut) {
        ...
        await sendEmail({
          ...
          body: `<div style="max-width: 600px;">
                  ...
                  <p style="font-size: 16px;"><srong>Hui EMS</strong></p>
                </div>`
        })
    ...
    if (leaveApplication?.status === "PENDING") {
        ...
        await sendEmail({
          ...
          body: `<div style="max-width: 600px;">
                  ...
                  <br />
                  <p style="font-size: 16px;">Best Regards,</p>
                  <p style="font-size: 16px;"><strong>Hui EMS</strong></p>
                </div>`
        })
    }
  }
...
// Cron: Check attendance at 11:30 AM IST (06:30 UTC) and email absent employees
const leaveApplicationReminderCron = inngest.createFunction(
  {id: "leave-application-reminder-cron", triggers: [{cron: "0 0 6 * * *"}]},  // 06:00 UTC = 11:30 AM IST
  async ({step}) => {
    // Step 1: Get today's date range (IST)
    const today = await step.run("get-today-date", () => {
      const startUTC = new Date(new Date().toLocaleDateString("en-CA", {timeZone: "Asia/Seoul"}) + "T00:00:00+05:30")
