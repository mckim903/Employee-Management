...
const Sidebar = () => {
  ...
  const sidebarContent = (
    <>
      {/* Brander Header */}
      <div className="px-5 pt-6 pb-5 border-r border-white/6">
        ...
        <div className="mx-3 mt-4 mb-1 p-3 rounded-lg bg-white/3 border border-white/4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-slate-800 flex items-center justify-center ring-1 ring-white/10 shrink-0">
                <span className="text-slate-400 font-semibold text-white/80">
      ...
      <div className="px-5 pt-5 pb-2">
        <p className="text-[11px] font-medium text-slate-500 uppercase tracking-[0.12em]">Navigation</p>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} to={item.href} className={`group flex items-center gap-3 px-3 py-2.5 rounded-md text-[13px] font-medium transition-all duration-150 relative ${isActive ? "bg-indigo-500/12 text-indigo-300" : "text-slate-300 hover:text-white hover:bg-white/4"}`}>
              ...
              <item.icon className={`w-[17px] h-[17px] shrink-0 ${isActive ? 'text-indigo-400' : 'text-slate-400 group-hover:text-slate-300'}`} />
  ...
  return (
    <>
      ...
      {/* Sidebar - desktop */}
      <aside className="hidden lg:flex flex-col w-[260px] h-full bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4">
        {sidebarContent}
      </aside>
