  ...
  const portalOptions = [
    {
      ...
      icon: ShieldIcon, // Replace with the actual icon component for admin
    },
    {
      to: '/login/employee',
      title: 'Employee Portal',
      subtitle: 'View your profile, track attendance, request time off, and access payslips.',
      icon: UserIcon, // Replace with the actual icon component for employee
    }
  ]
        ...
        <div className="w-full max-w-md animate-fade-in relative z-10">
          ...
          <div className="space-y-4">
            {portalOptions.map((portal, index) => (
              <Link key={index} 
                to={portal.to}
                className="group block bg-slate-50 border border-slate-200 rounded-lg p-5 sm:p-6 transition-all duration-300 hover:border-indigo-400 hover:lg-indigo-50"
              >
          ...
          <div className="mt-12 text-center md:text-left text-sm text-slate-400">
            <p>@ {new Date().getFullYear()} Hui Kim. All rights reserved.</p>
          </div>
        </div>
      </div>
    </div>
